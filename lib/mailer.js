    ...
    .map(
      ...
      <tr>
        <td style="padding:9px 14px;border-bottom:1px solid #EDE9E0;">
          <span style="font-family:'Segoe UI','Trebuchet MS',Tahoma,sans-serif;font-size:14px;color:${r.done ? "#9C947F" : INK};${r.done ? "text-decoration:line-through;" : ""}">
            ${r.emoji ? r.emoji + "&nbsp; " : ""}${escapeHtml(r.text)}
          </span>
          ${r.tag ? `<span style="float:right;font-family:'Segoe UI','Trebuchet MS',Tahoma,sans-serif;font-size:11px;color:${r.tagColor || AMBER_INK};font-weight:bold;">${escapeHtml(r.tag)}</span>` : ""}
        </td>
      </tr>`
    )
    .join("");
  ...
  <div style="background:${PAPER};padding:32px 16px;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;margin:0 auto;">
      <tr><td style="padding:0 4px 12px;">
        <span style="font-family:'Segoe UI','Trebuchet MS',Tahoma,sans-serif;font-size:14px;font-weight:bold;color:${INK};">NARCO<span style="color:${AMBER};">.</span></span>
      </td></tr>
      <tr><td style="background:#FFFFFF;border:1px solid #E7E3DA;border-radius:12px;overflow:hidden;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          ...
          <tr><td style="padding:22px 20px 6px;">
            <div style="font-family:'Segoe UI','Trebuchet MS',Tahoma,sans-serif;font-weight:bold;font-size:22px;line-height:1.2;color:${INK};">${escapeHtml(title)}</div>
            ${intro ? `<div style="font-family:'Segoe UI','Trebuchet MS',Tahoma,sans-serif;font-size:14px;color:${INK2};padding-top:8px;line-height:1.55;">${escapeHtml(intro)}</div>` : ""}
          </td></tr>
          ...
          <tr><td style="padding:16px 20px 20px;">
            <span style="font-family:'Segoe UI','Trebuchet MS',Tahoma,sans-serif;font-size:12px;color:${INK2};">${escapeHtml(footer || "Open the dashboard. Log it. Move.")}</span>
          </td></tr>
        </table>
      </td></tr>
      <tr><td style="padding:12px 4px;">
        <span style="font-family:'Segoe UI','Trebuchet MS',Tahoma,sans-serif;font-size:11px;color:${INK2};">The empire is built one day at a time.</span>
      </td></tr>
    </table>
  </div>`;
}
