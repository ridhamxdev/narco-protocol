...
export async function buildRangeDays(user, settings, habits, daysN) {
  ...
  const created = tzParts(tz, new Date(user.createdAt)).date;
  let from = addDays(today, -(daysN - 1));
  if (diffDays(from, created) < 0) from = created;
  const userId = user._id;
  const range = { $gte: from, $lte: today };

  const [hlogs, apps, gyms, foods, games, sleeps, persons, tasks, calls, pendingCalls] =
    await Promise.all([
...
export function dayNumber(user, settings) {
  const tz = settings.tz || DEFAULT_TZ;
  const created = tzParts(tz, new Date(user.createdAt)).date;
  return diffDays(protocolToday(tz), created) + 1;
}
...
export async function buildToday(user, settings) {
  ...
  const [hlogs, appsCount, recentApps, gym, foods, games, sleep, lastSleep, persons, tasks, pendingCalls, callsToday] =
    await Promise.all([
...
export async function buildStats(user, settings, daysN = 98) {
  ...
  return {
    days: days.map((d) => ({ date: d.date, pct: d.pct, total: d.total })),
    apps: {
      total: appsTotal,
      byStatus,
      byDay: last14.map((d) => ({ date: d.date, count: d.extras.apps })),
    ...
    clean: last7.map((d) => ({
      date: d.date,
      state: d.extras.mealsLogged === 0 ? -1 : d.extras.cleanDone ? 1 : 0,
    })),
    game: {
      limit: settings.gameLimit ?? 120,
      byDay: last14.map((d) => ({ date: d.date, min: d.extras.gameMin })),
    },
    sleep: last14.map((d) => ({ date: d.date, state: d.extras.sleepState })),
  };
}
