...
import {
  HabitLog, Application, Call, Task, FoodLog, GymLog, GameLog, SleepLog, PersonLog, SmokeLog, EmailLog,
} from "@/lib/db";
...
export const POST = api(async (req, ctx, user) => {
  ...
  const results = await Promise.all([
    ...
    PersonLog.deleteMany(q), SmokeLog.deleteMany(q), EmailLog.deleteMany(q),
  ]);
